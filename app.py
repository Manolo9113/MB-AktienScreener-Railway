...
Konkrete Asset-Allocation-Empfehlung: Was über-/untergewichten und warum? Unterscheide nach Zeithorizont (6 Monate taktisch vs. 3–5 Jahre strategisch). Besonderes Augenmerk auf Risiko/Rendite-Verhältnis im aktuellen Umfeld."""
    ...
    _MK_EVENTS = [
        ...
        # ── Mai 2026 ──
        ("2026-05-20", "Fed-Protokoll (FOMC Minutes)",         "Fed",  2),
        ("2026-05-22", "US PCE / Kerninflation (April)",       "CPI",  3),
        ("2026-05-27", "US BIP Q1 (2. Schätzung)",            "BIP",  2),
        ("2026-05-29", "US PCE Deflator (April)",              "CPI",  3),
        # ── Juni 2026 ──
        ("2026-06-04", "EZB Zinsentscheid",                    "EZB",  3),
        ("2026-06-05", "US Arbeitsmarkt (NFP Mai)",            "Jobs", 3),
        ("2026-06-10", "US CPI Mai",                           "CPI",  3),
        ("2026-06-16", "Fed Zinsentscheid (FOMC)",             "Fed",  3),
        ("2026-06-17", "Fed Pressekonferenz Powell",           "Fed",  3),
        ("2026-06-17", "US PPI Mai",                           "CPI",  2),
        ("2026-06-19", "US Michigan Sentiment",                "Stim", 1),
        ("2026-06-25", "US BIP Q1 (3. Schätzung)",            "BIP",  2),
        ("2026-06-26", "US PCE Deflator (Mai)",                "CPI",  3),
        # ── Juli 2026 ──
        ("2026-07-02", "US Arbeitsmarkt (NFP Juni)",           "Jobs", 3),
        ("2026-07-14", "US CPI Juni",                          "CPI",  3),
        ("2026-07-15", "US PPI Juni",                          "CPI",  2),
        ("2026-07-23", "EZB Zinsentscheid",                    "EZB",  3),
        ("2026-07-28", "Fed Zinsentscheid (FOMC)",             "Fed",  3),
        ("2026-07-29", "US BIP Q2 (1. Schätzung)",            "BIP",  3),
        ("2026-07-31", "US PCE Deflator (Juni)",               "CPI",  3),
        # ── August 2026 ──
        ("2026-08-07", "US Arbeitsmarkt (NFP Juli)",           "Jobs", 3),
        ("2026-08-12", "US CPI Juli",                          "CPI",  3),
        ("2026-08-13", "US PPI Juli",                          "CPI",  2),
        ("2026-08-26", "Fed Jackson Hole Symposium",           "Fed",  3),
        # ── September 2026 ──
        ("2026-09-04", "US Arbeitsmarkt (NFP August)",         "Jobs", 3),
        ("2026-09-09", "US CPI August",                        "CPI",  3),
        ("2026-09-10", "EZB Zinsentscheid",                    "EZB",  3),
        ("2026-09-15", "Fed Zinsentscheid (FOMC)",             "Fed",  3),
        ("2026-09-16", "Fed Pressekonferenz Powell",           "Fed",  3),
        ("2026-09-25", "US BIP Q2 (3. Schätzung)",            "BIP",  2),
        # ── Oktober 2026 ──
        ("2026-10-02", "US Arbeitsmarkt (NFP September)",      "Jobs", 3),
        ("2026-10-14", "US CPI September",                     "CPI",  3),
        ("2026-10-27", "Fed Zinsentscheid (FOMC)",             "Fed",  3),
        ("2026-10-29", "EZB Zinsentscheid",                    "EZB",  3),
        ("2026-10-29", "US BIP Q3 (1. Schätzung)",            "BIP",  3),
        # ── November 2026 ──
        ("2026-11-06", "US Arbeitsmarkt (NFP Oktober)",        "Jobs", 3),
        ("2026-11-12", "US CPI Oktober",                       "CPI",  3),
        # ── Dezember 2026 ──
        ("2026-12-04", "US Arbeitsmarkt (NFP November)",       "Jobs", 3),
        ("2026-12-08", "Fed Zinsentscheid (FOMC)",             "Fed",  3),
        ("2026-12-09", "US CPI November",                      "CPI",  3),
        ("2026-12-17", "EZB Zinsentscheid",                    "EZB",  3),
    ]
    _CAT_CLR = {
...
if st.session_state.get("show_etf_analyzer"):
    ...
    if _etf_raw_resolved:
        st.markdown(
            f"<div style='background:#0d2137;border:1px solid {_C_ACCENT}55;"
            f"border-left:3px solid {_C_ACCENT};border-radius:10px;"
            f"padding:10px 16px;margin:10px 0 4px 0;"
            f"display:flex;align-items:center;gap:10px;'>"
            f"<span style='font-size:1.1rem;'>⬇️</span>"
            f"<span style='color:{_C_TEXT_PRIMARY};font-size:0.82rem;'>"
            f"ETF <b style='color:{_C_ACCENT};'>{_etf_raw_resolved}</b> ausgewählt — "
            f"Analyse und ETF-spezifische Rechner weiter unten.</span>"
            f"</div>",
            unsafe_allow_html=True)
    ...
    # ── ETF Rechner (nur ohne ausgewählten ETF) ───────────────────────────
...
# ==================== TAB 2: PROGNOSE ====================
    ...
    # ── ABSCHNITT 6: Growth Chart (Innerer Wert vs. Kurs) ─────────────────────
    st.markdown(
    ...
    if _gc_inc is not None and not _gc_inc.empty:
        _gc_eps_row = next((r for r in ['Diluted EPS', 'Basic EPS'] if r in _gc_inc.index), None)
        _gc_ni_row  = next((r for r in ['Net Income', 'Net Income Common Stockholders',
    ...
    if len(_gc_metric) >= 2 and not _gc_hist.empty:
        ...
        st.markdown(
            ...
            f'ℹ️ Keine ausreichenden Daten für den Wachstums-Chart verfügbar.</div>',
            unsafe_allow_html=True)
